...
/*
    무한 순환참조 방지
    1. 발생 원인:
        BoardEntity <-> ReplyEntity 간 양방향 연관관계가 설정되어 있을 때,
        Lombok의 @ToString 으로 인해 서로를 무한으로 호출하여
        StackOverflowError 가 발생
    2. 해결 방법:
        @ToString(exclude = ...)
          > 연관관계 필드(@ManyToOne, @OneToMany)는 모두 Lombok 출력 대상에서 제외
        API 응답 시 Entity를 직접 반환하지 않고 DTO로 변환하여 반환하는 것을 권장
 */
...
public class BoardEntity {
	...
	/*
		@ManyToOne (N:1 단방향/양방향 연관관계)
		  - JPA에서 Entity 간 연관관계를 매핑할 때 외래키(Foreign Key)를 지정하는 어노테이션
		fetch = FetchType.LAZY (지연 로딩)
		  - DB의 연관된 Entity를 로딩하는 전략을 지정하는 속성
		  - 성능 최적화 권장
		  - 연관된 엔티티를 즉시 조회하지 않고, 실제 사용하는 시점에 DB 조회
		  - (EAGER 사용 시 불필요한 조인과 N+1 쿼리 폭탄 발생 가능성)
		@JoinColumn
		  - DB 테이블의 Foreign Key 컬럼명을 지정
		  - referencedColumnName은 대상 테이블은 외래키를 가짐
	...
	/*
		@OneToMany (1:N 양방향 연관관계)
		  - 하나의 엔티티가 여러 개의 엔티티를 참조
		  - mappedBy = "board" : ReplyEntity의 'board' 필드가 외래키를 관리함을 의미
		  - cascade = CascadeType.ALL : 연관 Entity 변경/삭제 시 해당 Entity도 변경/삭제
		  - orphanRemoval = true : List에서 제거된 엔티티 객체를 DB에서도 자동 DELETE 처리
	 */
	@OneToMany(mappedBy = "board", cascade = CascadeType.ALL, orphanRemoval = true)
	private List<ReplyEntity> replyList;        // 댓글 목록
	
	@OneToMany(mappedBy = "board", cascade = CascadeType.ALL, orphanRemoval = true)
	private List<BoardLikeEntity> likeList;      // 추천 이력
